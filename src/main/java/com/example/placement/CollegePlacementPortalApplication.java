                ...
                return (args) -> {
                        try {
                                ...
                                Student student = Student.builder()
                                                ...
                                                .pincode("400001")
                                                .department("CSE")
                                                // .collegeName(...) // Using default "National Institute of Technology,
                                                // Goa"
                                ...
                                Job job = Job.builder()
                                                ...
                                                .companyId(company.getId()) // Restricted @Setter field, but Builder
                                                                            // works
                                ...
                                System.out.println("Emails: " + student.getCollegeEmail() + " | "
                                                + student.getPersonalEmail());
                                System.out.println("Mobile: " + student.getMobile());
                                System.out.println(
                                                "Address: " + student.getCity() + ", " + student.getState() + " - "
                                                                + student.getPincode());
                                System.out.println("Education: " + student.getDepartment() + " at "
                                                + student.getCollegeName());
                                System.out.println(
                                                "Degree: " + student.getHighestDegree() + " | Year: "
                                                                + student.getCurrentYear()
                                                                + " | Passing: " + student.getYearOfPassing());
                                System.out.println("Performance: CGPA " + student.getCgpa());
                                System.out.println("Social: LinkedIn(" + student.getLinkedinUrl() + "), GitHub("
                                                + student.getGithubUrl()
                                                + "), Portfolio(" + student.getPortfolioUrl() + ")");
                                System.out.println(
                                                "Identity: PAN(" + student.getPanNumber() + "), Aadhaar("
                                                                + student.getAadhaarNumber() + ")");
                                System.out
                                                .println("Account: Status(" + student.getStatus() + ") | Password("
                                                                + student.getPassword() + ")");
                                System.out
                                                .println("Audit: Created(" + student.getCreatedAt() + "), Updated("
                                ...
                                System.out.println(
                                                "Contacts: Email(" + company.getOfficialEmail() + "), Phone("
                                                                + company.getOfficialPhone() + ")");
                                System.out.println("Website: " + company.getWebsite());
                                System.out.println(
                                                "Location: " + company.getCity() + ", " + company.getState() + ", "
                                                                + company.getCountry());
                                ...
                                System.out
                                                .println("Audit: Created(" + company.getCreatedAt() + "), Updated("
                                ...
                                System.out.println("HR Contacts: Email(" + job.getHrEmail() + "), Mobile("
                                                + job.getHrMobileNo()
                                                + ")");
                                System.out.println("Type & Mode: Offer(" + job.getOfferType() + "), Job("
                                                + job.getJobMode()
                                                + "), Drive("
                                                + job.getDriveMode() + ")");
                                System.out.println("Compensation: Stipend(" + job.getStipend() + "), CTC("
                                                + job.getCtcPackage()
                                                + ")");
                                System.out.println("Eligibility: Min CGPA(" + job.getMinimumCgpa() + "), Branches("
                                                + job.getBranchesAllowed() + "), Degrees(" + job.getEligibleDegree()
                                                + ")");
                                System.out.println("Graduating Years: BTech(" + job.getGraduatingYearBtech()
                                                + "), MTech("
                                                + job.getGraduatingYearMtech() + ")");
                                System.out.println("Details: Locations(" + job.getJobLocations() + "), Drive Date("
                                                + job.getDriveDate()
                                                + "), Openings(" + job.getNumberOfOpenings() + ")");
                                System.out.println("Process: Deadline(" + job.getApplicationDeadline()
                                                + "), Upcoming Event("
                                ...
                                System.out
                                                .println("Status: Job Status(" + job.getJobStatus() + ") | Verified("
                                                                + job.getIsVerified() + ")");
                                System.out.println("Audit: Created(" + job.getCreatedAt() + "), Updated("
                                                + job.getUpdatedAt()
                                                + ")");
                                System.out.println("--------------------------------------");
                                ...
                                System.out.println("Account: Username(" + admin.getUsername() + "), Email("
                                                + admin.getEmail()
                                                + "), Mobile(" + admin.getMobile() + ")");
                                System.out.println("Auth: Password(" + admin.getPassword() + "), Role("
                                                + admin.getRole()
                                                + "), Active("
                                                + admin.getIsActive() + ")");
                                System.out.println("Audit: Last Login(" + admin.getLastLoginAt() + "), Created("
                                                + admin.getCreatedAt()
                                ...
                                System.out.println(
                                                "Links: Job ID(" + application.getJobId() + "), Student ID("
                                                                + application.getStudentId() + ")");
                                System.out.println("Status: Application(" + application.getApplicationStatus()
                                                + "), Offer("
                                                + application.getOfferStatus() + ")");
                                // System.out.println("Details: Remark(" + application.getRemark() + "), Admin
                                // Verified("
                                ...
                                System.err.println("Error during demo data initialization: " + e.getMessage());
                                e.printStackTrace();
                        }
                };
        }
